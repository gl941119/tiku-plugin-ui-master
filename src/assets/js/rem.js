...
(function (win, lib) {
  ...
  function refreshRem () {
    const horizontalSceen = docEl.getBoundingClientRect().width > docEl.getBoundingClientRect().height
    let width = horizontalSceen ? docEl.getBoundingClientRect().height : docEl.getBoundingClientRect().width
    let rem = width / 7.5
    /** 平板应除以10 */
    if (OS.isTablet) {
      rem = width / 10
