import { defaultCookieExpire } from '@/config/common'
...
class Storage {
   ...
   * @param {json|int} options 相关设置,type=cookie时有效.为数字时表时cookie的过期时间为毫秒。
   * @property {string} otpions.path cookie可见地址
   * @property {string} options.domain cookie可见域
   * @property {boolean}  options.secure 是否支持https传输
   */
  [setCookie] (key, value, options) {
    ...
    if (Number.isInteger(options)) {
      opt.expires = options > 0 ? options : -1
    } else if (Object.isObject(options)) {
      opt = {
        ...opt,
        ...options
      }
      opt.expires = opt.expires > 0 ? opt.expires : opt.expires
    }
